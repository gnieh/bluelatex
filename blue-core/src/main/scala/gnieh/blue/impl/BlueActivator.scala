...
class BlueActivator extends ActorSystemActivator {
  ...
  def configure(context: BundleContext, system: ActorSystem): Unit =
    // the synchronization server is required
    for(syncServer <- context.get[SynchroServer]) {
      ...
      context.registerService(classOf[RestApi], new CoreApi(config, syncServer, templates.get, mailAgent, recaptcha), null)
    }
